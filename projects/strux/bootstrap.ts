...
import { createStruxJSAdapter, registerAgents, streamReply } from "@langgraph-toolkit/adapter-struxjs";

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const app = new Application(__dirname);
const { runtime, results } = await registerAgents(path.join(__dirname, "app", "Agents"));
const adapter = createStruxJSAdapter(runtime);
...
Route.post("/agents/:name/run", async (request: Request<Record<string, string>, Record<string, string>, JsonObject>) => {
  ...
  return adapter.runtime.run(name, body, { threadId: typeof body.threadId === "string" ? body.threadId : undefined });
});
Route.get("/agents/:name/stream", async (request: Request<Record<string, string>, Record<string, string>, JsonObject>, reply: Response) => {
  ...
  await streamReply(adapter.runtime, name, reply, input);
});
