...
async function runSpecialist(
  agent: {
    ...
    run(input: { query: string }): Promise<{ readonly output: { readonly content: string; readonly toolCalls: readonly { readonly name: string }[] } }>;
  },
  query: string,
): Promise<Partial<ChatMcpState>> {
  const { output } = await agent.run({ query });
  return { agentResults: { [agent.name]: { agent: agent.name, content: output.content, toolCalls: output.toolCalls.map(({ name }) => name) } } };
}
...
export function createChatMcpWorkflow(
  ...
  return createWorkflow("chat-mcp", { state: ChatState })
    ...
    .route({ data: "sql", research: "research", general: "draft", action: "sql" }, { field: "intent" })
